	...
	MuzGameEngineStorageModel* pRucksack)
{
	...
	const int emergencyMoveHorizon = pRucksack->m_engineSettings.GetOptionByKey("Emergency_Move_Horizon");	// 何か急ぐんかな（＾～＾）？
	const int nokositeokuTime      = pRucksack->m_engineSettings.GetOptionByKey("Minimum_Thinking_Time");	// 手番で、使わずに残しておく思考時間☆
	...
	if (pRucksack->m_engineSettings.GetOptionByKey("USI_Ponder")) {
