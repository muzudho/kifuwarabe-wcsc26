...
public:
	...
	virtual inline void explorePlain_n200n650_returnWithValue(
		...
		const Depth depth,
		ScoreNumber& beta,
		ScoreNumber& eval) const override
	{
		// PVノードはスルー☆！（＾ｑ＾）
	}
		...
		Key& posKey) const override
	{
			...
			)
		{
			...
			//────────────────────────────────────────────────────────────────────────────────
			// PVノードの場合☆
			g_NODEKIND_PROGRAMS[NodeKind::No1_PV]->explorePlain_10a520b500c500d500e100f(
	...
	/// <param name="mp"></param>
	/// <param name="cutNode"></param>
	inline void explorePlain_n700n750_forkNewMonkey(
		bool& isBreak,
		OurCarriage& ourCarriage,
		const Depth depth,
		Monkey** ppThisThread,
