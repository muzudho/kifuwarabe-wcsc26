...
/// 指し手生成区分：合法手生成
/// </summary>
class MovegenTypeLegal : public MovegenTypeAbstract {
...
public:
	...
	DeliciousBanana* GenerateMove(DeliciousBanana* moveStackList, const Position& pos, bool all = false
		...
		{
			moveStackList = MovegenTypeLegal::GENERATE_MOVE_<Color::Black, Color::White>(moveStackList, pos, all);
		}
		else
		{
			moveStackList = MovegenTypeLegal::GENERATE_MOVE_<Color::White, Color::Black>(moveStackList, pos, all);
		}
...
private:
	...
	static inline DeliciousBanana* GENERATE_MOVE_(DeliciousBanana* moveStackList, const Position& pos, bool all = false
		...
		moveStackList = pos.InCheck() ?
			MovegenTypeEvasion().GenerateMove(moveStackList, pos) :
