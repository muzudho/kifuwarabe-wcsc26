...
/// 指し手生成区分：王手回避
/// </summary>
class MovegenTypeEvasion : public MovegenTypeAbstract {
...
public:
	...
	DeliciousBanana* GenerateMove(DeliciousBanana* moveStackList, const Position& pos, bool all = false
		...
		{
			moveStackList = MovegenTypeEvasion::GENERATE_MOVE_<Color::Black, Color::White>(moveStackList, pos, all);
		}
		else
		{
			moveStackList = MovegenTypeEvasion::GENERATE_MOVE_<Color::White, Color::Black>(moveStackList, pos, all);
		}
...
private:
	...
	static inline DeliciousBanana* GENERATE_MOVE_(DeliciousBanana* moveStackList, const Position& pos, bool all = false
		...
		do {
			...
			assert(PieceExtensions::TO_COLOR10(pos.GetPiece(checkSq)) == THEM);
