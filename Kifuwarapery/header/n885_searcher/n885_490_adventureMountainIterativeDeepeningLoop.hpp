...
public:
	...
	static inline void ExploreMountain(MuzGameEngineStorageModel& ourCarriage, Position& pos)
	{
		...
		// マルチＰＶの数☆？
		ourCarriage.m_pvSize = ourCarriage.m_engineOptionCollection.m_map["MultiPV"];
		AdventurerSkill skill(ourCarriage.m_engineOptionCollection.m_map["Skill_Level"], ourCarriage.m_engineOptionCollection.m_map["Max_Random_Score_Diff"]);

		if (ourCarriage.m_engineOptionCollection.m_map["Max_Random_Score_Diff_Ply"] < pos.GetGamePly()) {
		...
		while (++depth <= g_maxPly && !ourCarriage.m_signals.m_stop && (!ourCarriage.m_limits.m_depth || depth <= ourCarriage.m_limits.m_depth)) {
			...
			if (
				...
				if (
					...
					//────────────────────────────────────────────────────────────────────────────────
					// さあ、非PV探索に潜るぜ～☆！（＾ｑ＾）　２回目のぐるんぐるんだぜ～☆ ノンＰＶ～☆
					//────────────────────────────────────────────────────────────────────────────────
