		...
		while (++depth <= g_maxPly && !ourCarriage.m_signals.m_isIterationDeepingStop && (!ourCarriage.m_limits.m_depth || depth <= ourCarriage.m_limits.m_depth)) {
			...
			for (ourCarriage.m_pvIdx = 0; ourCarriage.m_pvIdx < ourCarriage.m_pvSize && !ourCarriage.m_signals.m_isIterationDeepingStop; ++ourCarriage.m_pvIdx) {
				...
				while (true) {
					...
					//────────────────────────────────────────────────────────────────────────────────
					bestScore = g_NODEKIND_PROGRAMS[NodeKind::No0_Root]->explorePlain_10a520b500c500d500e100f(ourCarriage, pos, flashlight + 1, alpha, beta, static_cast<Depth>(depth * OnePly), false);
			...
			if (
				...
				if (
					...
					const ScoreNumber s = g_NODEKIND_PROGRAMS[NodeKind::No2_NonPV]->explorePlain_10a520b500c500d500e100f(
