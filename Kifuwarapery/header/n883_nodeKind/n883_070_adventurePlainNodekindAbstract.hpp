...
public:
	...
	virtual inline void explorePlain_10i1260j_byEasyGoingMonkey(
			...
			&& abs(beta) < SweetnessMateInMaxPly)
		{
            ...
            // ［大雑把な性格の猿］が静止探索☆！（＾ｑ＾）
			const Sweetness s = AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalities::EasyGoing]->ExploreAsQsearch(
	...
	virtual inline void explorePlain_10i1280j_byEasyGoingMonkey(
			...
			&& abs(beta) < SweetnessMateInMaxPly)
		{
			...
			Sweetness nullSweetness = (depth - reduction < OnePly ?
				...
				//────────────────────────────────────────────────────────────────────────────────
				-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalities::EasyGoing]->ExploreAsQsearch(
		...
		Sweetness& beta) const
	{
			...
			&& Sweetness::SweetnessMatedInMaxPly < bestSweetness)
		{
			...
			if (depth < 16 * OnePly
				...
				this->lock_10i400j170k300L(ppMonkeySplitedPlace);
				isContinue = true;
				return;
			}
	...
	virtual inline void lockAndUpdateBestSweetness_10i400j170k200L(
		...
		(*ppMonkeySplitedPlace)->m_mutex.lock();
		if ((*ppMonkeySplitedPlace)->m_bestSweetness < bestSweetness) {
			(*ppMonkeySplitedPlace)->m_bestSweetness = bestSweetness;
		}
	}
		...
		if (willFullDepthSearch) {
			...
			sweetness = (
				...
				?
					(givesCheck
					?
						-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalities::EasyGoing]->ExploreAsQsearch(ourCarriage, true, pos, (*ppFlashlight) + 1, -(alpha + 1), -alpha, Depth0)
					:
						-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalities::EasyGoing]->ExploreAsQsearch(ourCarriage, false, pos, (*ppFlashlight) + 1, -(alpha + 1), -alpha, Depth0)
					)
				:
		...
		if (
			...
			sweetness = (
				...
				?
					(givesCheck
					?
						-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalities::Meticulous]->ExploreAsQsearch(ourCarriage, true, pos, (*ppFlashlight) + 1, -beta, -alpha, Depth0)
					:
						-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalities::Meticulous]->ExploreAsQsearch(ourCarriage, false, pos, (*ppFlashlight) + 1, -beta, -alpha, Depth0)
					)
				:
		...
		) const
	{
		...
		if (isPVMove || alpha < sweetness) {
			...
			{
				rm.m_sweetness_ -= m_engineSettings["Danger_Demerit_Score"];
			}
#endif
