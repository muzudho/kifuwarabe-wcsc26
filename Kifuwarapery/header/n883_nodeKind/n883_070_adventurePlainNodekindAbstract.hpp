...
public:
	...
	virtual inline void explorePlain_10i1260j_byEasyGoingMonkey(
			...
			&& abs(beta) < SweetnessMateInMaxPly)
		{
            ...
            // ［大雑把な性格の猿］が静止探索☆！（＾ｑ＾）
			const Sweetness s = AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalitiesForQuiescenceSearch::QsearchEasyGoing]->ExploreAsQsearch(
	...
	virtual inline void explorePlain_10i1280j_byEasyGoingMonkey(
			...
			&& abs(beta) < SweetnessMateInMaxPly)
		{
			...
			Sweetness nullSweetness = (depth - reduction < OnePly ?
				...
				//────────────────────────────────────────────────────────────────────────────────
				-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalitiesForQuiescenceSearch::QsearchEasyGoing]->ExploreAsQsearch(
		...
		if (willFullDepthSearch) {
			...
			sweetness = (
				...
				?
					(givesCheck
					?
						-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalitiesForQuiescenceSearch::QsearchEasyGoing]->ExploreAsQsearch(ourCarriage, true, pos, (*ppFlashlight) + 1, -(alpha + 1), -alpha, Depth0)
					:
		...
		if (
			...
			sweetness = (
				...
				?
					(givesCheck
					?
						-AdventureBattlefieldQsearchByMonkeyPersonalitiesModel::m_pAdventureBattlefieldQsearchByMonkeyPersonalities[MonkeyPersonalitiesForQuiescenceSearch::QsearchMeticulous]->ExploreAsQsearch(ourCarriage, true, pos, (*ppFlashlight) + 1, -beta, -alpha, Depth0)
					:
