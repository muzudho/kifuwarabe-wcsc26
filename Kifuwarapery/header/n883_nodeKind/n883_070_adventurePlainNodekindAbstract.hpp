...
public:
	...
	/// <param name="threatMove"></param>
	/// <param name="bestMove"></param>
	virtual inline void explorePlain_10a520b500c500d500e500f500g200h150i_setMoveNone(
		ScoreNumber& bestScore,
		Flashlight** ppFlashlight,
		Move& threatMove,
		Move& bestMove) const
	...
	/// <param name="pFlashlight"></param>
	virtual inline void explorePlain_10a520b500c500d500e500f500g200h200i_updateMaxPly(
		Monkey** ppThisThread,
	...
	/// <param name="pTtEntry"></param>
	/// <returns></returns>
	virtual inline DeliciousBanana explorePlain_10a520b500c500d500e500f500g200h405i_getTtDeliciousBanana(
		Move& excludedMove,
		Flashlight** ppFlashlight,
		Key& posKey,
		Position& pos,
		...
		Move& ttMove) const
	{
		...
		if (pTtEntry != nullptr
			...
			return std::make_tuple(true, ttScore);
		}
		...
		Flashlight** ppFlashlight) const
	{
		...
		{
			const ScoreNumber rbeta = beta - ourCarriage.razorMargin(depth);
			const ScoreNumber s = AdventureBattlefieldQsearchPrograms::m_pAdventureBattlefieldQsearchPrograms[No2_NonPV]->ExploreAsQsearch(
		...
		Move& threatMove) const
	{
			...
			&& abs(beta) < ScoreMateInMaxPly)
		{
			...
			ScoreNumber nullScore = (depth - reduction < OnePly ?
				//────────────────────────────────────────────────────────────────────────────────
				...
				//────────────────────────────────────────────────────────────────────────────────
				-g_NODEKIND_PROGRAMS[NodeKind::No2_NonPV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight) + 1, -beta, -alpha, depth - reduction, !cutNode)
			);
			...
			if (beta <= nullScore) {
				...
				// 探索☆？（＾ｑ＾）
				//────────────────────────────────────────────────────────────────────────────────
				const ScoreNumber s = g_NODEKIND_PROGRAMS[NodeKind::No2_NonPV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight), alpha, beta, depth - reduction, false);
		...
		const bool cutNode) const
	{
		...
		{
			const ScoreNumber rbeta = beta + 200;
			...
			while (!(move = mp.GetNextMove_NonSplitedNode()).IsNone()) {
				if (
					...
					// 探索☆？（＾ｑ＾）
					//────────────────────────────────────────────────────────────────────────────────
					score =	-g_NODEKIND_PROGRAMS[NodeKind::No2_NonPV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight) + 1, -rbeta, -rbeta + 1, rdepth, !cutNode);
					pos.UndoMove(move);
					if (rbeta <= score) {
						isReturnWithScore = true;
						return;
	...
	virtual inline void explorePlain_10a520b500c500d500e500f500g300h900i_loopHeader(
		Depth& extension,
	...
	virtual inline void explorePlain_10a520b500c500d500e500f500g400h500i_recursiveSearch(
		OurCarriage& ourCarriage,
		bool& givesCheck,
		Position& pos,
		Move& move,
	...
	{
		if (givesCheck && ScoreNumber::ScoreZero <= pos.GetSeeSign(move))
		{
			...
			&& abs(ttScore) < PieceScore::m_ScoreKnownWin)
		{
			...
			const ScoreNumber rBeta = ttScore - static_cast<ScoreNumber>(depth);
			...
			// 探索☆？（＾ｑ＾）
			//────────────────────────────────────────────────────────────────────────────────
			score =	g_NODEKIND_PROGRAMS[No2_NonPV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight), rBeta - 1, rBeta, depth / 2, cutNode);
		...
		ScoreNumber& beta) const
	{
		...
		if (!captureOrPawnPromotion
			...
			&& ScoreNumber::ScoreMatedInMaxPly < bestScore)
		{
			assert(move != ttMove);
	...
	/// <param name="givesCheck"></param>
	/// <param name="ppFlashlight"></param>
	virtual inline void explorerPlain_10a520b500c500d500e500f500g500h100i_doMove(
		Position& pos,
		Move& move,
		StateInfo& st,
		const CheckInfo& ci,
		...
		Flashlight** ppFlashlight) const
	{
		...
		((*ppFlashlight) + 1)->m_staticEvalRaw.m_p[0][0] = ScoreNumber::ScoreNotEvaluated;
	}
	...
	virtual inline void explorePlain_10a520b500c500d500e500f500g500h500i_recursiveSearch(
		OurCarriage& ourCarriage,
		const Depth depth,
		bool& isPVMove,
		bool& captureOrPawnPromotion,
		...
		bool& doFullDepthSearch) const
	{
			...
			&& (*ppFlashlight)->m_killers[1] != move)
		{
			...
			//────────────────────────────────────────────────────────────────────────────────
			// PVS
			score = -g_NODEKIND_PROGRAMS[No2_NonPV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight) + 1, -(alpha + 1), -alpha, d, true);
	...
	/// <param name="alpha"></param>
	/// <param name="cutNode"></param>
	virtual inline void explorePlain_10a520b500c500d500e500f500g500h600i_nonPVRecursiveSearch(
		OurCarriage& ourCarriage,
		bool& doFullDepthSearch,
		ScoreNumber& score,
		Depth& newDepth,
		...
		const bool cutNode) const
	{
		...
		if (doFullDepthSearch) {
			score = (newDepth < OnePly ?
				...
				// 探索☆？（＾ｑ＾）
				//────────────────────────────────────────────────────────────────────────────────
				: -g_NODEKIND_PROGRAMS[No2_NonPV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight) + 1, -(alpha + 1), -alpha, newDepth, !cutNode));
		}
	}
		...
		Flashlight** ppFlashlight) const
	{
		...
		if (
			...
			score = (newDepth < OnePly ?
				...
				// 再帰的探索☆？（＾ｑ＾）
				//────────────────────────────────────────────────────────────────────────────────
				: -g_NODEKIND_PROGRAMS[No1_PV]->explorePlain_10a520b500c500d500e500f500g_entry(ourCarriage, pos, (*ppFlashlight) + 1, -beta, -alpha, newDepth, false));
		}
	}
	...
	/// <param name="pos"></param>
	/// <param name="move"></param>
	virtual inline void explorePlain_10a520b500c500d500e500f500g700h100i_undoMove(
		Position& pos,
		Move& move) const
	{
		pos.UndoMove(move);
	...
	/// </summary>
	/// <returns></returns>
	virtual inline bool explorePlain_10a520b500c500d500e500f500g800h400i_isReturn() const = 0;
		...
		Move movesSearched[64]) const
	{
		...
		assert(-ScoreNumber::ScoreInfinite < bestScore && bestScore < ScoreNumber::ScoreInfinite);
	}
