...
Bitboard RookAttackBb::MakeRookBlockMask(const Square square) const {
	...
	//         .
	//
    // 交点を除くために、縦横のマスクを XOR している。また、端のマスクを消すために、縦横のマスクを AND している。
	//
	Bitboard result = g_fileMaskBb.GetSquareFileMask(square) ^ g_rankMaskBb.GetSquareRankMask(square);

    if (ConvSquare::TO_FILE10(square) != FileA) { result &= ~g_fileMaskBb.GetFileMask(FileA); }	// ９筋のマスクを消す
    if (ConvSquare::TO_FILE10(square) != FileI) { result &= ~g_fileMaskBb.GetFileMask(FileI); }	// １筋のマスクを消す
    if (ConvSquare::TO_RANK10(square) != Rank1) { result &= ~g_rankMaskBb.GetRankMask(Rank1); } // ９段のマスクを消す
    if (ConvSquare::TO_RANK10(square) != Rank9) { result &= ~g_rankMaskBb.GetRankMask(Rank9); } // １段のマスクを消す
	return result;
}
